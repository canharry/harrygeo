...
@section('title', $tag->name . ' - 标签归档 - 阳光每一天')

@section('content')
    <!-- 标签归档页 Hero 小横幅 -->
    <section class="page-hero" style="--page-hero-color: {{ $tag->color ?? '#667eea' }}">
            ...
            <div class="card tag-card">
                ...
                <div class="tag-cloud">
                    @foreach ($tags as $item)
                        <a href="{{ route('tags.show', $item->slug) }}" class="tag-item {{ $item->id === $tag->id ? 'active' : '' }}" style="--tag-color: {{ $item->color ?? '#667eea' }}">{{ $item->name }}</a>
                    @endforeach
                </div>
            </div>
        </aside>
                            ...
                            <div class="post-body">
                                ...
                                <div class="post-tags">
                                    @foreach ($post->tags->take(3) as $t)
                                        <a href="{{ route('tags.show', $t->slug) }}" class="post-tag {{ $t->id === $tag->id ? 'active' : '' }}" style="--tag-color: {{ $t->color ?? '#667eea' }}">{{ $t->name }}</a>
                                    @endforeach
                                </div>
                                <div class="post-meta">
