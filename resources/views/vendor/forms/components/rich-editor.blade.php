            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        @if ($hasToolbarButton(['videoUpload', 'videoLink']))
                            <div
                                data-trix-button-group="video-tools"
                                class="flex items-stretch space-x-1 rtl:space-x-reverse"
                            >
                                @if ($hasToolbarButton('videoUpload'))
                                    <x-forms::rich-editor.toolbar-button
                                        data-trix-action="x-video-upload"
                                        title="上传视频"
                                        tabindex="-1"
                                    >
                                        <svg
                                        ...
                                        >
                                            <path
                                                fill="currentColor"
                                                d="M0 128C0 92.7 28.7 64 64 64H320c35.3 0 64 28.7 64 64V384c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V128zM559.1 99.8c10.4 5.6 16.9 16.4 16.9 28.2V384c0 11.8-6.5 22.6-16.9 28.2s-23 5-32.9-3.4l-96-64L416 337.1V320 192 174.9l14.2-9.5 96-64c9.8-6.5 22.4-7.2 32.9-3.4z"
                                            ></path>
                                        </svg>
                                    </x-forms::rich-editor.toolbar-button>
                                @endif

                                @if ($hasToolbarButton('videoLink'))
                                    <x-forms::rich-editor.toolbar-button
                                        data-trix-action="x-video-link"
                                        title="插入视频链接"
                                        tabindex="-1"
                                    >
                                        <svg
                                        ...
                                        >
                                            <path
                                                fill="currentColor"
                                                d="M579.8 267.7c56.5-56.5 56.5-148 0-204.5c-50-50-128.8-56.5-186.3-15.4l-1.6 1.1c-14.4 10.3-17.7 30.3-7.4 44.6s30.3 17.7 44.6 7.4l1.6-1.1c32.1-22.9 76-19.3 103.8 8.6c31.3 31.3 31.3 82.1 0 113.4L457.3 364.7c-31.3 31.3-82.1 31.3-113.4 0c-28.6-28.6-32.1-72.5-8.6-103.8l1.1-1.6c10.3-14.4 7.1-34.4-7.4-44.6s-34.4-7.1-44.6 7.4l-1.1 1.6C211.5 286.7 215.9 357.4 267.7 409.2c56.5 56.5 148 56.5 204.5 0L579.8 267.7zM60.2 244.3c-56.5 56.5-56.5 148 0 204.5c50 50 128.8 56.5 186.3 15.4l1.6-1.1c14.4-10.3 17.7-30.3 7.4-44.6s-30.3-17.7-44.6-7.4l-1.6 1.1c-32.1 22.9-76 19.3-103.8-8.6C74.4 354.1 74.4 303.3 105.7 272l96.1-96.1c31.3-31.3 82.1-31.3 113.4 0c28.6 28.6 32.1 72.5 8.6 103.8l-1.1 1.6c-10.3 14.4-7.1 34.4 7.4 44.6s34.4 7.1 44.6-7.4l1.1-1.6C433.5 225.3 429.1 154.7 377.3 102.9c-56.5-56.5-148-56.5-204.5 0L60.2 244.3z"
                                            ></path>
                                        </svg>
                                    </x-forms::rich-editor.toolbar-button>
                                @endif
...
@once
    <script>
        (function () {
            ...
            window.__trixVideoToolsInitialized = true;
            ...
            function insertVideoPlaceholder(editor, url, type, filename) {
                var html = '<p><a href="' + escapeHtml(url) + '">▶ 视频：' + escapeHtml(filename) + '</a></p>';
                editor.insertHTML(html);
            }

            function handleVideoUpload(editor) {
                ...
                input.onchange = function () {
                    ...
                    fetch('/posts/upload-video', {
                        ...
                        .then(function (data) {
                            if (data.url) {
                                insertVideoPlaceholder(editor, data.url, 'html5', file.name);
                            } else {
                                alert(data.message || '视频上传失败');
                            }
                        })
                        .catch(function () {
                            alert('视频上传失败，请稍后重试');
            ...
            function handleVideoLink(editor) {
                var url = prompt('请输入视频链接（支持 YouTube、Bilibili 或直接视频地址）：');
                if (!url) {
                    return;
                }

                var type = detectVideoType(url);
                var filename = type === 'youtube' ? 'YouTube 视频' : (type === 'bilibili' ? 'Bilibili 视频' : '外部视频');
